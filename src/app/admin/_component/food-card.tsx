  ...
  return (
    <div className="w-full aspect-[4/3] h-fit border rounded-3xl p-5">
      <div
        ...
        className="rounded-3xl w-full h-2/3 mb-4 bg-cover bg-center relative"
      >
        <Dialog>
          <DialogTrigger>
            <div className="rounded-full w-11 h-11 absolute bottom-4 right-4 text-[#EF4444] bg-background flex justify-center items-center">
              <Pencil size={16} />
            </div>
          </DialogTrigger>
          <DialogContent>
            ...
            <form className="mb-4 mt-4 gap-4 items-start">
              <div className="grid grid-cols-[20%_76%] w-full gap-4 mb-4 ">
                    ...
                    <SelectGroup>
                      {categories.map((category: CategoryType) => (
                        <SelectItem
                          key={category.id}
                          value={category.id}
                          className="bg-secondary pl-3 pr-3 rounded-full dark:hover:bg-gray-800"
                        >
                          {category.name}
                        </SelectItem>
                      ))}
                ...
                <textarea
                  name="Ingredients"
                  className="border rounded-lg w-full p-2 h-24"
                  placeholder="List ingredients..."
                  defaultValue={info.ingredients}
                ></textarea>
                <div className="text-muted-foreground text-sm">Dish image</div>
                <label
                  htmlFor="dropzone-file"
                  className="flex flex-col items-center justify-center w-full aspect-[3/1] border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 overflow-hidden"
                >
