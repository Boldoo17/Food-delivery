  ...
  return (
    <div className="whitespace-nowrap">
      <Badge className="rounded-full p-2">
        {title}
        <div className="rounded-full bg-black text-white ml-2 pl-2 pr-2 text-xs">
          {counted}
        </div>
      </Badge>
    </div>
...
export function CategoryCard({
  ...
  async function getFoods() {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/food-category/${id}/foods`
    );
  ...
  return (
    <div className="w-full h-fit rounded-2xl bg-background p-3 grid grid-cols-4 gap-3">
      <h2 className="font-extrabold text-2xl col-span-4">
        {title}&nbsp;({foods.length})
      </h2>
      <div className="w-full h-full border rounded-3xl p-2 border-dashed border-[#EF4444] flex flex-col items-center justify-center">
        <Dialog>
          ...
          <DialogContent>
            ...
            <form
              className="mb-4 mt-4 flex flex-col gap-5 items-start"
              action={addFood}
            >
              ...
              <label
                ...
                className="flex flex-col items-center justify-center w-full aspect-[3/1] border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 overflow-hidden"
              >
      ...
      {foods.map((food: Food) => (
        <FoodCard key={food._id} info={food} categories={categories} />
      ))}
    </div>
  );
}
